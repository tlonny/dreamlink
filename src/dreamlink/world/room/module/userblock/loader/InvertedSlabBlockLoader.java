...
import dreamlink.world.room.module.userblock.InvertedSlabBlock;
...
public class InvertedSlabBlockLoader {
    ...
    public InvertedSlabBlockLoader(IRoomModuleProvider provider) {
        this.provider = provider;
        this.isSolidParser = new MaterialIsSolidParser(InvertedSlabBlockLoader.defaultIsSolid);

        this.isTranslucentParser = new LightIsTranslucentParser(InvertedSlabBlockLoader.defaultIsLightTransmitting);
        this.isAffectedParser = new LightIsAffectedParser(InvertedSlabBlockLoader.defaultIsAffectedByLighting);

        this.iconTextureSampleParser = new TextureSampleParser(
            this.provider, 
            String.format(InvertedSlabBlockLoader.textureSampleKeyTemplate, InvertedSlabBlockLoader.iconTextureSampleSuffix),
        ...
        for(var ix = 0; ix < CubeFace.getSize(); ix += 1) {
            ...
            this.faceTextureSampleParsers[ix] = new TextureSampleParser(
                this.provider, 
                String.format(InvertedSlabBlockLoader.textureSampleKeyTemplate, cubeFace.name)
            );
        }
    }

    public InvertedSlabBlock loadInvertedSlabBlock(File file) throws RoomLoadException {
        try {
            ...
            var block = new InvertedSlabBlock(
                this.provider,
                blockID,
                file.getName(),
                ...
                blockConfig.optFloat(InvertedSlabBlockLoader.soundGainKey, InvertedSlabBlockLoader.defaultGain)
            );
